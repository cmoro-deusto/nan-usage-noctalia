...
- `LICENSE` ships inside the plugin directory, because the plugin is
  GPL-2.0-or-later rather than MIT. There is no repository-wide license covering a
  contributed plugin directory.
- `catalog.toml` is generated by their CI: never add or edit one in a pull request.

## License

GPL-2.0-or-later, the same license as the command line tool this plugin drives.
The plugin directory carries its own copy because the store asks for one when a
plugin is not MIT.
